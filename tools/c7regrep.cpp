...
static void chmod_same(const std::string& target, const std::string& ref)
{
    struct stat st;
    if (::stat(ref.c_str(), &st) == C7_SYSOK) {
	::chmod(target.c_str(), st.st_mode & 0777);
    }
}
...
repl_copy(const char *source, const c7::strvec& lines, ctx_t& ctx)
{
    ...
    size_t size_io = 0;
    if (auto res = c7::file::read(source, size_io); !res) {
	c7echo(res);
	return RESULT_ERROR;
    } else {
	...
	std::string path = source;
	path += ".bck";
	if (auto res = c7::file::write(path, 0644, up.get(), size_io); !res) {
	    c7echo(res);
	    return RESULT_ERROR;
	}
	chmod_same(path, source);
