 ...
 */
#ifndef __C7_THREADCOM_HPP_LOADED__
#define __C7_THREADCOM_HPP_LOADED__
...
private:
    ...
    inline com_status put_framework(::timespec *tmp, Assign assign) {
	auto defer = cv_.lock();
	while (state_ == EXIST) {
	    if (!cv_.wait(tmp)) {
...
public:
    ...
    pipe1& operator=(const pipe1<T>&) = delete;
	...
	return put_framework(tmp, [&](){ data_ = data; });
    }

    com_status put(T&& data, ::timespec *tmp = nullptr) {
	return put_framework(tmp, [&](){ data_ = std::move(data); });
    }
