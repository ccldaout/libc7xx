...
public:
    ...
    template <typename T>
    result<size_t> read(T& buf) {
	return read(&buf, sizeof(buf));
    }
    template <typename T>
    result<size_t> write(const T& buf) {
    ...
    template <typename T>
    io_result read_n(T& buf) {
	return read_n(&buf, sizeof(buf));
    }
    template <typename T>
    io_result write_n(const T& buf) {
	return write_n(&buf, sizeof(buf));
    }
