...
#undef  _IHDRSIZE
#define _IHDRSIZE		c7_align(sizeof(hdr7_t), 16)
		      ...
		      std::function<bool(const info_t&)>& choice)
{
	...
	c7::usec_t log_beg = hdr_->log_beg;
	auto& rd = readers.emplace_back(log_beg << 20, rbufs_[i], i);
...
make_mlog_reader7()
{
    return std::make_unique<mlog_reader7>();
}
